...
comment on column foodgroup_code is '4-digit code indicating food group to which a food item belongs.';
comment on column long_desc is '200-character description of food item.';
comment on column short_desc is '60-character abbreviated description of food item. Generated from the 200-character description using abbreviations in Appendix A. If short description is longer than 60 characters, additional abbreviations are made.';
comment on column common_name is 'Other names commonly used to describe a food, including local or regional names for various foods, for example, “soda” or “pop” for “carbonated beverages.”';
comment on column mfg_name is 'Indicates the company that manufactured the product, when appropriate.';
comment on column survey is 'Indicates if the food item is used in the USDA Food and Nutrient Database for Dietary Studies (FNDDS) and thus has a complete nutrient profile for the 65 FNDDS nutrients.';
comment on column ref_desc is 'Description of inedible parts of a food item (refuse), such as seeds or bone.';
comment on column pefuse is 'Percentage of refuse.';
comment on column sciname is 'Scientific name of the food item. Given for the least processed form of the food (usually raw), if applicable.';
comment on column n_factor is 'Factor for converting nitrogen to protein (see p. 12).';
comment on column pro_factor is 'Factor for calculating calories from protein (see p. 14).';
comment on column fat_factor is 'Factor for calculating calories from fat (see p. 14).';
comment on column cho_factor is 'Factor for calculating calories from carbohydrate (see p. 14).';
comment on column fdgrp_code is '4-digit code identifying a food group. Only the first 2 digits are currently assigned. In the future, the last 2 digits may be used. Codes may not be consecutive.';
comment on column foodgroup_desc is 'Name of food group.';
